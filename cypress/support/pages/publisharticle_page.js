    ...
    deleteTestArticleIfExists()
        ...
        fillingTheArticleFieldsTotally(){
        ...
        clearPublishArticleInputField(field){
        const fields = {
        ...
        clickToPublishTheArticle(){
            cy.get("#post").click()
        },
        iSeeTheMessageOfArticleCreationPage(message){
            cy.get("#articleModalBox",{timeout:5000}).should('be.visible')
            cy.contains('#articleModalBox p', message).should("be.visible")
            cy.contains("button", "Fechar").click()

        },
        checkingTheCreatedArticle(){
            
        cy.contains('h1', "Artigos", {timeout:5000}).should('be.visible')
        cy.contains("h2", "Test Article").parent().contains("a", "Ler mais").click()
        cy.contains('h1', "Test Article", {timeout:5000}).should('be.visible')
        cy.get('img.articleImage[alt="Test Article"]').should('be.visible')
        cy.get('.articleText p').eq(0).should('be.visible').and('have.text', 'Paragraph test')
        cy.get('.articleText p').eq(1).should('be.visible').and('have.text', 'Paragraph test 2')
        cy.get('.articleText p').eq(2).should('be.visible').and('have.text', 'Paragraph test 3')

        },
        cancelingArticlePublishing(){
            cy.contains("button", "Cancelar").click()
            cy.contains("h2", "O que deseja fazer?", {timeout:5000}).should('be.visible')
        },
        chekingThatTheArticleWasNotCreated(){
        cy.contains('h1', "Artigos", {timeout:5000}).should('be.visible')
