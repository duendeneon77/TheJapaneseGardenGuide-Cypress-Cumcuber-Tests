export default{
    editToroSessionPage(){
    cy.contains('h3', 'Editar página Toro',{timeout:5000}).should('be.visible')
    cy.get('#textHistory').click().type('{end}\n\n <img src="/testImages/testimage.png" alt=""/>')
    cy.get('#textHistory').click().type("{end}\n\nEDIT-TORO-SESSION-TEST-TEXT")
    },
    saveTheEdition(){
    ...
    cleanupToroSessionPage() {
    const imageTag = '<img src="/testImages/testimage.png" alt=""/>'
    const testText = "EDIT-TORO-SESSION-TEST-TEXT"
