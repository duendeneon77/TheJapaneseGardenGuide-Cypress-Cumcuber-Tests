export default{
    editBridgesSessionPage(){
    cy.contains('h3', "Editar página Bridges",{timeout:5000}).should('be.visible')
    cy.get('#textHistory').click().type('{end}\n\n <img src="/testImages/testimage.png" alt=""/>')
    cy.get('#textHistory').click().type("{end}\n\nEDIT-BRIDGES-SESSION-TEST-TEXT")
    },
    saveTheEdition(){
    ...
    cleanupBridgesSessionPage() {
    const imageTag = '<img src="/testImages/testimage.png" alt=""/>'
    const testText = "EDIT-BRIDGES-SESSION-TEST-TEXT"
